...
from utils.units import ft2m, in2m, rad2Deg
...
ELEV_MIN_DRAW_DIST = in2m(8.0)  
#^^Elevator cannot retract all the way to the bottom - the "zero" point is somewhere up along the angle
...
class _ElevatorMech():
    ...
    def _getElevDrawLen(self, lenIn):
        return lenIn + ELEV_MIN_DRAW_DIST
    
    def _getSingerSupportDrawAngle(self, angleIn):
        return angleIn - ELEV_MOUNT_ANGLE_DEG + 90.0

    def __init__(self, root, isDesired):

        size = 3 if isDesired else 6

        elevatorColor = GHOST_COLOR if isDesired else ELEV_COLOR
        singerColor = GHOST_COLOR if isDesired else SINGER_COLOR
        suffix = "_des" if isDesired else "_act"

        self.elev = root.appendLigament(f"Elevator{suffix}", self._getElevDrawLen(0.0), 
                                         ELEV_MOUNT_ANGLE_DEG, size, elevatorColor)
        self.singerSupport = self.elev.appendLigament(f"SingerSupport{suffix}", SINGER_OFFSET_DIST, 
                                                       0.0, size, singerColor)
        self.singerFwd = self.singerSupport.appendLigament(f"SingerFwd{suffix}", SINGER_FWD_DIST,
                                                            -90.0, size, singerColor)
        self.singerRev = self.singerSupport.appendLigament(f"SingerRev{suffix}", SINGER_REV_DIST, 
                                                           90.0, size, singerColor)
